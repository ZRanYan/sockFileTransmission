...
int md5File(const char *filename,char *md5)
{
	...
	if(!fp)
    {
		...
		while((input_size = fread(input_buffer, 1, 1024, fp)) > 0){
	...
	char tmp[3]={0};
	for(input_size=0;input_size<16;input_size++){
		sprintf(tmp,"%02x",ctx.digest[input_size]);
		memcpy(&md5[input_size*2],tmp,2);
	}
	md5[32]='\0';
	return 0;
}
